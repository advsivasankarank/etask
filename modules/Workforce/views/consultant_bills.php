<section class="panel">
    ...
    <form method="get" action="<?= e(url('/workforce/consultant-bills')) ?>" class="search-bar">
        <select name="status" style="padding:14px 15px;border:1px solid #d8e1eb;border-radius:12px;"><option value="">All Statuses</option><?php foreach (['DRAFT','SUBMITTED','APPROVED','PAID','REJECTED'] as $s): ?><option value="<?= e($s) ?>" <?= ($filters['status'] ?? '') === $s ? 'selected' : '' ?>><?= e(label_case($s)) ?></option><?php endforeach; ?></select>
    ...
    <?php if (($bills['items'] ?? []) === []): ?><div class="data-card" style="text-align:center;padding:40px;"><div class="eyebrow">No Bills</div><p class="subtle" style="margin:8px 0 0;">No bills found.</p></div><?php else: ?>
        ...
        <?php foreach ($bills['items'] as $b): ?><tr><td><strong><?= e($b['consultant_name'] ?: '-') ?></strong></td><td><?= e($b['bill_no'] ?: '-') ?></td><td><?= e($b['bill_date'] ?: '-') ?></td><td><?= e(money_inr($b['amount'] ?? 0)) ?></td><td><?= e(money_inr($b['tax_amount'] ?? 0)) ?></td><td><?= e(money_inr($b['total_amount'] ?? 0)) ?></td><td><span class="chip <?= $b['status'] === 'PAID' ? '' : ($b['status'] === 'REJECTED' ? 'chip-strong' : '') ?>"><?= e(label_case((string) $b['status'])) ?></span></td><td><?php if (\App\Core\Auth::can('workforce.consultants.manage')): ?><form method="post" action="<?= e(url('/workforce/consultant-bills/status')) ?>" style="display:inline;"><?= \App\Core\Csrf::inputField() ?><input type="hidden" name="bill_id" value="<?= e((string) $b['id']) ?>"><select name="status" onchange="this.form.submit()" style="padding:4px 8px;font-size:0.78rem;border:1px solid #d8e1eb;border-radius:8px;"><option value="">Update</option><?php foreach (['DRAFT','SUBMITTED','APPROVED','PAID','REJECTED'] as $s): ?><option value="<?= e($s) ?>"><?= e(label_case($s)) ?></option><?php endforeach; ?></select></form><?php endif; ?></td></tr><?php endforeach; ?>
        </tbody></table></div>
    <?php endif; ?>
</section>
