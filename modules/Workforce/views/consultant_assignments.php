<section class="panel">
    ...
    <form method="get" action="<?= e(url('/workforce/consultant-assignments')) ?>" class="search-bar">
        <select name="status" style="padding:14px 15px;border:1px solid #d8e1eb;border-radius:12px;"><option value="">All Status</option><?php foreach (['ASSIGNED','IN_PROGRESS','DELIVERED','APPROVED','REWORK','CANCELLED'] as $s): ?><option value="<?= e($s) ?>" <?= ($filters['status'] ?? '') === $s ? 'selected' : '' ?>><?= e($s) ?></option><?php endforeach; ?></select>
    ...
    <?php if (($assignments['items'] ?? []) === []): ?><div class="data-card" style="text-align:center;padding:40px;"><div class="eyebrow">No Assignments</div><p class="subtle" style="margin:8px 0 0;">No assignments found.</p></div><?php else: ?>
        ...
        <?php foreach ($assignments['items'] as $a): ?><tr><td><strong><?= e($a['consultant_name'] ?: '-') ?></strong></td><td><?= e($a['assignment_title']) ?></td><td><?= e($a['so_no'] ?: '-') ?></td><td><?= e($a['client_name'] ?: '-') ?></td><td><?= e($a['due_date'] ?: '-') ?></td><td><span class="chip"><?= e($a['status']) ?></span></td><td><?= e($a['fee_agreed'] ? 'INR ' . number_format((float) $a['fee_agreed'], 2) : '-') ?></td><td><?php if (\App\Core\Auth::can('workforce.consultants.manage')): ?><form method="post" action="<?= e(url('/workforce/consultant-assignments/status')) ?>" style="display:inline;"><?= \App\Core\Csrf::inputField() ?><input type="hidden" name="assignment_id" value="<?= e((string) $a['id']) ?>"><select name="status" onchange="this.form.submit()" style="padding:4px 8px;font-size:0.78rem;border:1px solid #d8e1eb;border-radius:8px;"><option value="">Update</option><?php foreach (['ASSIGNED','IN_PROGRESS','DELIVERED','APPROVED','REWORK','CANCELLED'] as $s): ?><option value="<?= e($s) ?>"><?= e($s) ?></option><?php endforeach; ?></select></form><?php endif; ?></td></tr><?php endforeach; ?>
        </tbody></table></div>
    <?php endif; ?>
</section>
