<section class="panel">
    ...
    <?php if (($movements['items'] ?? []) === []): ?><div class="data-card" style="text-align:center;padding:40px;"><div class="eyebrow">No Movements</div><p class="subtle" style="margin:8px 0 0;">No DSC movements recorded.</p></div><?php else: ?>
        ...
        <?php foreach ($movements['items'] as $mov): ?><tr><td><strong><?= e($mov['holder_name'] ?: '-') ?></strong></td><td><?= e($mov['client_name'] ?: '-') ?></td><td><span class="chip"><?= e(label_case((string) $mov['movement_type'])) ?></span></td><td><?= e($mov['from_user_name'] ?: $mov['from_location'] ?: '-') ?></td><td><?= e($mov['to_user_name'] ?: $mov['to_location'] ?: '-') ?></td><td><?= e($mov['movement_date']) ?></td><td><span class="chip <?= $mov['status'] === 'OPEN' ? '' : 'chip-strong' ?>"><?= e(label_case((string) $mov['status'])) ?></span></td><td><?php if ($mov['status'] === 'OPEN' && \App\Core\Auth::can('dsc.movement.manage')): ?><form method="post" action="<?= e(url('/dsc/movement/return')) ?>" style="display:inline;"><?= \App\Core\Csrf::inputField() ?><input type="hidden" name="movement_id" value="<?= e((string) $mov['id']) ?>"><button type="submit" class="button button-secondary" style="padding:4px 8px;font-size:0.78rem;">Return</button></form><?php endif; ?></td></tr><?php endforeach; ?>
        </tbody></table></div>
    <?php endif; ?>
</section>
